...
class DetailsFrontend:
    def __init__(self,Curseur,InfosFacture):
        ...
        self.Supprimer_btn= Button(self.Actions,bg='brown',text='Supprimer',fg='white',command=self.effacerOperation)
    ...
    def save(self):
        self.selectCombo
        print(self.InfosFacture[0],self.IdentifiantArticle,self.QuantEnt.get())
        self.DataDetails=DetailsBackend(self.InfosFacture[0],self.IdentifiantArticle,self.QuantEnt.get())
        self.DataDetails.save(self.curseur)
        self.Actualiser()
        self.ReseteInput()
    def effacerOperation(self):
        # print product id
        print("self.InfosFacture[0]:", self.InfosFacture[0])
        print("self.IdentifiantArticle:", self.IdentifiantArticle)
        print("self.QuantEnt.get():", self.QuantEnt.get())
        data = DetailsBackend(self.InfosFacture[0],self.IdentifiantArticle,self.QuantEnt.get())
        data.deleteData(cursor=self.curseur)
        self.Actualiser()
        self.ReseteInput()
