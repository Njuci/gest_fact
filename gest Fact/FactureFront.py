...
class FactureFrontend:
    def __init__(self,Curseur):
        ...
        self.Facture=FactBackend(self.idFacEnt.get(),self.DateEnt.get(),self.RedEnt.get(),self.IdCliEnt.get(),self.curseur)
        Data =  self.Facture.getData()
        print(Data)
    ...
    def AjoutsDetails (self):
        if self.idFacEnt.get()=='' and self.DateEnt.get()=='' and self.RedEnt.get()=='':
            showerror('BAC4 INFO','Veuillez selectionner la facture')
        else:
