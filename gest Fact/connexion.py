...
class DetailsBackend:
    ...
    def save(self,cursor):
        ...
        try:
            cursor.execute("insert into vente values(%s,%s,%s)",(self.idArt,self.idfact,self.Quant))
    ...
    def UpdateData(self,cursor):
        ...
        try:
            ...
            showerror('BAC4 INFO', str(exp))
    def deleteData(self,cursor):
        self.curseur=cursor
        try:
            sql = 'DELETE FROM vente WHERE codArt = %s AND idfact=%s '
            val = (self.idArt,self.idfact)
            self.curseur.execute(sql,val)
            showinfo('BAC4 INFO','Suppression reçu')
        except Exception as exp :
            showerror('BAC4 INFO', str(exp))
